...
const HighScoreItem = ({ rank, model }: { rank: number, model: ModelInfo }) => {
	...
	console.log(rankStr.length + scoreStr.length + initalStr.length);
	...
	return (
		<div className="w-full">
			<RetroText color={model.color}>
				{rankStr}
				{scoreStr}
				<Tooltip content={model.fullName}>
					{initalStr}
				</Tooltip>
			</RetroText>
		</div>
...
export default function HighScoresPage() {
	return (
		<>
			<ColorHelper />
			<div className="bg-black w-svw min-h-svh overflow-x-hidden flex flex-col justify-center">
				<div>
					<Gap />
					<Blink interval={1000}>
					...
					<Labels />
					<Gap />
					{models.sort((a, b) => b.score - a.score).map((model, index) => (
